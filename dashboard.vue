<template>
  <v-container fluid>
    <v-row justify="center" class="my-5">
      ...
      <v-col cols="12" md="8">
        <v-card class="pa-5" style="border-radius: 20px; background-color: #e0f7fa;" hover outlined>
          <v-card-title class="text-center">
            <h3 class="headline font-weight-bold" style="color: #01579b;">ระบบฟาร์มมอนใหม</h3>
          </v-card-title>
          <v-card-text>
            <v-row>
              <!-- อุณหภูมิ -->
              <v-col cols="12" sm="6">
                <v-card style="border-radius: 20px; background-color: #bbdefb;" class="pa-3" hover outlined>
                  <v-card-title class="text-center" style="color: #0277bd;">อุณหภูมิปัจจุบัน</v-card-title>
                  <v-card-text class="text-center">
                    <v-progress-circular :model-value="value_temp" size="150" width="20" color="blue">
              ...
              <!-- ความชื้น -->
              <v-col cols="12" sm="6">
                <v-card style="border-radius: 20px; background-color: #e3f2fd;" class="pa-3" hover outlined>
                  <v-card-title class="text-center" style="color: #0288d1;">ความชื้นปัจจุบัน</v-card-title>
                  <v-card-text class="text-center">
            ...
            <v-row class="mt-5">
              <!-- อุณหภูมิที่เหมาะสม -->
              <v-col cols="12" sm="6">
                <v-card style="border-radius: 20px; background-color: #b3e5fc;" class="pa-3" hover outlined>
                  <v-card-title class="text-center" style="color: #039be5;">อุณหภูมิที่เหมาะสม</v-card-title>
                  <v-card-text class="text-center">
                    <v-progress-circular :model-value="totalValueT()" size="150" width="20" color="blue">
                      {{ totalValueT() }} °C
                    </v-progress-circular>
                    <v-row class="mt-3">
              ...
              <!-- ความชื้นที่เหมาะสม -->
              <v-col cols="12" sm="6">
                <v-card style="border-radius: 20px; background-color: #81d4fa;" class="pa-3" hover outlined>
                  <v-card-title class="text-center" style="color: #03a9f4;">ความชื้นที่เหมาะสม</v-card-title>
                  <v-card-text class="text-center">
                    <v-progress-circular :model-value="totalValueH()" size="150" width="20" color="blue">
                      {{ totalValueH() }} %
                    </v-progress-circular>
                    <v-row class="mt-3">
                      <v-col cols="6" class="text-center">
    ...
    <v-row justify="center" class="my-5">
      <!-- control Card -->
      <v-col cols="12" md="8">
        <v-card style="border-radius: 20px; background-color: #e1f5fe;" class="pa-5" hover outlined>
          <v-row justify="space-around" class="mb-5">
            <v-col cols="12" sm="5">
              <v-btn block color="blue darken-2" class="white--text" large @click="start" style="border-radius: 20px; height: 50px;">
                เปิด
              </v-btn>
            </v-col>
            <v-col cols="12" sm="5">
              <v-btn block color="blue lighten-1" class="white--text" large @click="stop" style="border-radius: 20px; height: 50px;">
                ปิด
              </v-btn>
            </v-col>
          </v-row>
          <div align="center" style="font-size: 21px; font-weight: bold; color: #0288d1;">ช่วงวัย</div>
          <v-card-text>
            <v-row class="justify-center">
              <v-col cols="3" class="text-center">
                <v-icon :color="isOn ? 'yellow' : 'grey'" style="font-size: 48px; margin: 5px;">
                  mdi-lightbulb-on
                </v-icon>
                <div style="color: #0288d1;">ON</div>
              </v-col>
              <v-col cols="2" class="text-center">
                <v-icon :color="numericValueState() === 1 || numericValueState() === 4 || numericValueState() === 5 ? 'red' : 'grey'" style="font-size: 38px; margin: 5px;">
                  mdi-lightbulb
                </v-icon>
              </v-col>
              <v-col cols="2" class="text-center">
                <v-icon :color="numericValueState() === 2 || numericValueState() === 4 || numericValueState() === 5 ? 'green' : 'grey'" style="font-size: 38px; margin: 5px;">
                  mdi-lightbulb
                </v-icon>
              </v-col>
              <v-col cols="2" class="text-center">
                <v-icon :color="numericValueState() === 3 || numericValueState() === 5 ? 'blue' : 'grey'" style="font-size: 38px; margin: 5px;">
                  mdi-lightbulb
                </v-icon>
              </v-col>
              <v-col cols="3" class="text-center">
                <v-icon :color="!isOn ? 'red' : 'grey'" style="font-size: 48px; margin: 5px;">
                  mdi-lightbulb
                </v-icon>
                <div style="color: #d32f2f;">OFF</div>
              </v-col>
            </v-row>
            <v-row class="justify-center mt-4">
              <v-col cols="6" class="text-center">
                <v-icon :color="temperatureStatus() === 'cold' || temperatureStatus() === 'quite cold' ? 'blue' : 'grey'" style="font-size: 48px; margin: 5px;">mdi-air-conditioner</v-icon>
                <div style="color: #0288d1;">เครื่องปรับความร้อน</div>
              </v-col>
              <v-col cols="6" class="text-center">
                <v-icon :color="humidityStatus() === 'hot' || humidityStatus() === 'quite hot' ? 'blue' : 'grey'" style="font-size: 48px; margin: 5px;">mdi-water</v-icon>
                <div style="color: #0288d1;">เครื่องพ่นหมอก</div>
              </v-col>
            </v-row>
            <v-divider class="my-5"></v-divider>
            <v-row class="mt-5">
              <v-col cols="12" sm="6">
                <h3 class="font-weight-bold" style="color: #01579b;">อุณหภูมิ</h3>
                <h3 class="text-center">{{ value_temp }} °C</h3>
              </v-col>
              <v-col cols="12" sm="6">
                <h3 class="font-weight-bold" style="color: #01579b;">ความชื้น</h3>
...
export default {
  ...
  methods: {
      ...
      onMqttMessage(topic, message) {
          ...
          //if (topic === 'node/datatotal' && this.isOn) {
              //let data1 = JSON.parse(message.toString());
              //this.value_temp = parseFloat(data1.temp);
              //this.value_humi = parseFloat(data1.humi);
              //this.value_state = parseInt(data1.state);
